...
#: 觀察點**完整文字**的上限。身分與跨日回顧都用完整文字(見
#: `normalized_trigger`),而 schema 對 `trigger` 沒有長度限制 —— 一個
#: 三萬字的 trigger 會被存進 state、活最多 28 天、每天原樣送進 packet
#: (外審 2026-08-17 r4 的 resource-boundary)。
#:
#: **數字是量出來的,不是猜的**:2026-08-17 生產帳本 4 條觀察點,長度
#: 16 / 28 / 38 / 39 字(中位 38、最長 39)—— 120 的顯示上限從來沒有真的
#: 截斷過任何一條。400 = 實測最長的十倍,仍留給複合條件(「若 A 且 B、
#: 則觀察 C」)充足空間,而 8 條開著的觀察點最多也只佔 3,200 字。
#: **超過不靜默截斷,而是進驗證問題 → 語意修補**:截斷會讓身分變成
#: 「前 N 字」,那正是這一輪剛拆掉的坑。
WATCH_TRIGGER_MAX = 400
