...
_GENERATED_CLASS = re.compile(r"s\d+\Z")
    ...
    while pos < end:
        ...
        if quote in "\"'":
            ...
            if name in seen and name in {"class", "style"}:
                return tag
            seen.add(name)
            if name == "class" and quote == '"' and _GENERATED_CLASS.fullmatch(value):
                replacements.append((open_quote, pos, value))
            elif (name == "style" and value.endswith(";")
                  and not _ENTITY_END.search(value)):
                replacements.append((value_start, close_quote, value[:-1]))
        else:
