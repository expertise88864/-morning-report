...
#: **「態勢」動作**(縱深第五批,2026-08-10):鍵(型別:動作:對象:月)
#: 本身就是那條線的身分,同鍵之下**不做逐樁切分**。
#:
#: 判準是「狀態 vs 行為」:hormuz_passage / strait_tension 指的是一個
#: **持續存在、逐日演變的狀態**(通行、緊張),選舉與峰會是一個**跨週的
#: 過程** —— 同對象同月不可能是「兩樁」。而軍售/攻擊/制裁/管制/關稅是
#: **會完成的行為**,同月對同一對象可以有兩筆不同的案子,逐樁切分
#: (incident_tokens)正是為它們設計的。
#:
...
SITUATION_ACTIONS = frozenset({
    "hormuz_passage", "strait_tension", "election", "summit_talks"})
