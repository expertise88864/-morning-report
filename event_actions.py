...
#: **持續態勢**(縱深第五批,2026-08-10;外審 P1-1 收窄):鍵(型別:動作:
#: 對象:月)
#: 本身就是那條線的身分,同鍵之下**不做逐樁切分**。
#:
#: 判準是**三分**,不是二分(外審 P1-1 的修正):
#:
#:   1. **持續態勢**(這張表):hormuz_passage / strait_tension 指的是一個
#:      持續存在、逐日演變的狀態(通行、緊張)。同對象同月只有一條線。
#:   2. **會重複發生的回合**:election / summit_talks 看起來像「過程」,
#:      實際上同對象同月可以有**兩件不同的事** —— 同一國同月的國會選舉
#:      與公投、同一組對手同月的貿易談判與安全會談。上一版把它們放進這張
#:      表,等於宣告「法國八月只會有一場選舉」。它們回到逐樁切分:
#:      同一場選舉的第一輪與第二輪靠辨識詞重疊接得上,兩件不同的事接不上
#:      —— 這正是 incident_tokens 的用途。
#:   3. **會完成的行為**(軍售/攻擊/制裁/管制/關稅):同月對同一對象可以
#:      有兩筆不同的案子,一律逐樁。
#:
#: 分不清楚的一律放第 2/3 類:漏接只是少算幾天(讀者看到「第 1 天」),
#: 誤併是把兩件事寫成同一個故事 —— 代價不對稱。
#:
#: 把切分套在態勢上的實害(2026-08-10 實信):荷莫茲 08-05 的禁航與
#: 08-10 的談判條件,標題辨識詞天然重疊很低(同一條線的**發展**本來就
#: 會換詞),NO_MATCH → 另開 `#04d558` 第 1 天 —— 「第 N 天」的連續性
#: 正是 timeline 存在的目的,而它在最需要的地方被切斷。
SITUATION_ACTIONS = frozenset({"hormuz_passage", "strait_tension"})
