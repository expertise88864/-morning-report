...
_KNOWN = {
    ...
    **{t: (f"US:EQUITY:{t}", EQUITY) for t in (
        ...
        "MTD",
        # 2026-08-18 使用者要求把涵蓋面補到 **NASDAQ-100 權重前段班**
        # (「有新聞才報」——宣告只是讓它們**能**被談,不是保證每天出現)。
        # 這四檔不在原本的「半導體鏈與大型科技股」範圍內:`COST`(零售)、
        # `TMUS`(電信業者)不是科技股,`CSCO`(網通設備)、`ADBE`(軟體)是
        # —— 是不是科技由 `industry_class` 判,不是「有沒有被宣告」。
        "COST", "CSCO", "TMUS", "ADBE",
    )},
