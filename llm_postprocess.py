...
#: 新聞不再全部掉進「其他類股」)。
RENDERER_VERSION = 27  # 2026-09-08:section integration, four tech stories and separated company headings.
...
#: 標籤後面允許掛一個**計分內部細節**的括號(2026-08-27 實信:
#: 「立場:中性(系統計分淨分 +3)」整行進了信 —— 淨分是後台計算,prompt
#: 本來就禁止它出現,而這個變體先前不匹配「標籤自成一行」的判準)。
#: **括號只認計分殘骸**(r1 外審):第一版收任意 0–30 字括號,於是
#: 「立場:中性(等待聯準會訊號)」這種**帶理由的括號**整行被砍 ——
#: 與上面「帶理由的句子是內容」的契約自相矛盾。要含「淨分/系統計分」
#: 才算殘骸。
...
#: 模型自加的**信頭殘留**(2026-08-27 實信,三行一組進了結論卡):
#:   晨報 2026/08/27
#:   立場:中性(系統計分淨分 +3)     ← 上面那條收
#:   11維計分行:                      ← 標題詞孤行(計分行本身已被拿掉)
#: 「晨報+日期」孤行與「11維計分行」孤行都是格式殘骸,不是內容。
#: **日期在前也算**(2026-08-28 實信):第一版只寫了「晨報 2026/08/27」
#: 的順序,而那天模型寫的是 `## 2026-08-28 晨報` —— 同一種殘骸、
#: 換個排法就漏掉,而且被 `_md_to_html` 渲染成 24px 大標題進了信裡。
#: 日期本身抽成一個片段,兩種排法共用。
_DATE_FRAG = r"\d{4}\s*[/\-年]\s*\d{1,2}\s*[/\-月]\s*\d{1,2}\s*日?"
_HEADER_DEBRIS_RE = (r"(?m)^[>\s]*[*_#]{0,3}\s*(?:"
                     r"晨報\s*" + _DATE_FRAG
                     + r"|" + _DATE_FRAG + r"\s*晨報"
                     r"|11\s*維計分行\s*[:：]?"
                     r")\s*[*_#]{0,3}\s*$")
...
def _strip_stance_scaffolding(text: str, instructions: str = "") -> str:
    ...
    for ln in text.split("\n"):
        ...
        if _re.search(_HEADER_DEBRIS_RE, ln):
            continue                    # 模型自加的信頭殘骸(晨報日期/計分行標題)
