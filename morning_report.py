...
※ **批#27(顯示規則):此區僅供你「內部判斷方向」,報告中一律不得出現「淨動能 ±N」「升X降Y」「[分析師評等動能]」等原始字樣,也不得只憑分析師評等動能單獨寫成一條;分析師動向至多作為某條**既有具體事件**(產品/合約/財報/法說/公告/籌碼)的「一句方向性佐證」,且不得引用動能數字。籌碼/法人買超等其他 B 級訊號仍依 R12 辦理、不受此限(本規則只約束「分析師評等動能」)。**
...
- **鐵則 1**:機制與信心標(A/B 級・信心)仍**必須保留**,敘事只是包裝,不可因說故事
  而省略「傳導到 2330/00662 的機制」或稀釋信心分級。
...
**台積電自家動態優先且可加深**(本報核心追蹤標的):新聞素材中凡屬 2330 自家的**財報/月營收數字、
