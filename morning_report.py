...
def _llm_config_resolved() -> dict:
    ...
    return {
        ...
        "LLM_REQUEST_TIMEOUT_SECONDS": _llm_request_timeout(),
...
#: 歸屬表要涵蓋**與 prompt 實際注入相同的事件集合**。
#: r1(Codex,P2):原本歸屬只做前 12 個,而 structured_news_block 注入 25 個
#: —— 第 13–25 個沒有指定主段落,不受「只在指定段落深寫一次」約束,
#: 跨段重複的問題在那些事件上原封不動。
STRUCTURED_EVENTS_IN_PROMPT = 25


def assign_event_sections(events, tw0050=None,
                          limit: int = STRUCTURED_EVENTS_IN_PROMPT) -> list:
    ...
    """
    # r1(Codex,P2):**重用既有的科技產業集合**,不要另造子集。
...
def _build_prompt(quotes: dict, fair: dict, predictions: dict,
    ...
    structured_news_block = json.dumps(
        [_sanitize_event_for_prompt(e)
         for e in (quotes.get("STRUCTURED_NEWS_EVENTS")
                   or [])[:STRUCTURED_EVENTS_IN_PROMPT]
    ...
    # r1(Codex,P2):**政策也要有唯一完整版歸屬。** 深度解析段對每項高分政策
    # 要求 6-10 行完整解析,而九段又被要求涵蓋金融/房市政策 —— 同一項政策
    # 會被完整寫兩次,正是本批要消除的重複。
    # 與上面那句同樣**條件化**:沒有深度解析段時提到它,會讓模型以為政策
    # 已在別處寫過而整段略過(那個坑上面那行註解已經記過一次)。
    policy_owner_note = (
        "**政策的唯一完整版歸屬**:被列入下方深度解析段的政策"
        "(信用管制、房市政策、金管會措施、稅制等)**完整版屬於該段**;"
        "本段對同一項政策**最多一句帶過並指向該段**,"
        "**不得重複展開內容、數字或傳導機制**。"
...
""" if weekly_review_block else "")
    ...
    # 第十四輪:寫作鐵律 R1–R16b 搬到 `writing_rules.py`。**逐字相同**
    # (golden prompt SHA 與 contract 行為雜湊都沒變)。搬的理由是使用者
    # 兩天內改了兩批寫法,而每一批都要同時動這裡與 `prompt_profiles.py`;
    # 埋在兩萬多行中段的 f-string 裡,兩邊很容易漂開。
    _writing_rules = _wr.LEGACY_RULES.format(
...
**只寫今日真的有新證據的項目**:沒有新證據的項目**整條不要出現**;
全部都沒有新證據時**整段省略**。
**鐵則**:昨日部分只能引用上方【昨日本報敘事回顧】確有的內容,**不可**替昨日補記它沒說過的話;今日部分必須引用今日新聞/數據。
昨日事件多半是**外電原始標題**,寫進信裡時要照 R10c **翻成中文轉述**——轉述只能改語言,不能改意思,也不能補上原標題沒有的內容。
...
3. **量級＋時間＋信心**：**不是**「利多/利空/中性」那個方向詞。要回答
   **「多大、多快、憑什麼是這個量級」**——例:「CoWoS 月產能約增一成，
   最快第四季反映在 2330 稼動率」。
   **真的判斷不出量級時就明講「這則新聞說不出量級，因為沒有金額/數量/時程」**，
   那是誠實;寫「小幅利多」則是用一個形容詞冒充答案。
**禁止**只寫「影響中性」「中性偏正」「有帶動作用」這類沒有機制的空話——那等於沒分析。
**同樣禁止**用方向形容詞當結論收尾:「小幅利多」「間接正面」「影響有限」
「暫中性」「僅方向性利多」「基本面傳導有限」——**這些是標籤不是分析**。
(2026-08-04 實測:本段 10 條有 10 條以方向詞作結、0 條說得出量級、
0 條與當天其他條目有關聯,使用者兩次反映「只是在堆疊數據」。)

**橫向連結(至少 2 條)**:本段不是十個孤島。至少兩條要寫出
**「這條跟今天哪一條同向或牴觸」**——例:「這與上一條日月光的設備採購指向
同一個先進封裝瓶頸」「但九段的運價回升暗示成本端反而在升，兩者方向相反」。
與鐵則 3 不衝突:鐵則 3 禁的是**同一件事**重複展開,這裡要的是**不同事件之間**
的關係,那正是「分析」與「條列」的差別。

**句式不得雷同**:十條全部長成「對 X 偏多、對 Y 影響有限」就是模板填空。
每條依它**自己最重要的那件事**決定怎麼收尾——有的收在量級、有的收在時間、
有的收在「什麼情況會推翻」、有的收在與另一條的矛盾。

**沒有真正公司新聞時的正確處理**：不要硬湊 8-12 檔、逐一只報「盤後漲跌 X%」充數（那是失敗報告）。
改寫 **2-3 個產業主題**（如「AI 伺服器拉貨」「記憶體報價」「先進製程稼動率」），引用上方新聞 / 8-K 的具體數字；真的沒料就誠實少寫幾條。

每條格式（嚴格遵守）：
**公司中英文名（一句話業務簡介）**：**(有前情時)半句承接昨日/近期這條線** ＋ [今日事件＋數字＋來源] ＋ [傳導機制] ＋ **[量級與時間,或明說量級判斷不出來]** ＋ **資訊強度(A/B)＋信心(高/中/低)**
（承接語見 R16:只在上方歷史/昨日回顧確有前情時寫,沒有就直接寫今日事件,不可捏造。）
**下面兩個範例就是本段的示範。** 它們刻意各自收在不同的地方(一個收在量級與
時間、一個收在「量級判斷不出來」加上與另一條的關係)——**照著模仿的是它們的
寫法,不是它們的句型**。

範例 A 級(具體事實＋機制＋量級與時間):
**Broadcom（AVGO，全球前三大半導體 IP 設計商，主導 AI ASIC 客製晶片）**：宣布獲 Anthropic 80 億美元算力訂單，AVGO 盤後 +4.5%。ASIC 量產倚賴台積電 N3 與 CoWoS，而 CoWoS 產能目前仍是瓶頸。以這個金額推估，相當於台積電先進封裝一個季度出貨量的個位數百分比，最快要到明年上半年才會反映在稼動率上——今天先動的是訂單能見度，不是營收。**[A 級・信心:高]**

範例 B 級(事件較軟;**誠實說量級判斷不出來**,並指出與另一條的關係):
**NVIDIA(NVDA,GPU/AI 加速器龍頭)**：財報週前多家雲端商釋出擴大 AI 伺服器採購的說法，惟無單一訂單數字。GPU 拉貨增量會經 CoWoS 與 HBM 傳導到 2330 稼動率，但**這則新聞說不出量級**——沒有金額、沒有數量、沒有時程，所以只能當方向。值得注意的是它與上一條的 ASIC 訂單指向同一段先進封裝產能，兩者互相排擠而不是相加。**[B 級・信心:中-低,資訊有限]**
...
(承接語型如「延續…這條線」「對比鮮明:…」「…的背後線索」)。
本段每一條都要寫成這樣,而不是各自獨立的公告。

**深度鐵則(每條必須三段式因果鏈,否則就是填充垃圾——與八段同一條規矩)**:
1. **事件**:發生什麼——政策/財報/合約/併購/運價/新藥進度/車市數據/報價/標案 ＋ 具體數字 ＋ 來源媒體。
2. **傳導機制**:為何牽動哪一檔或哪一類——必須點名**具體機制**
   (壽險投資收益 / 銀行淨息差 / 運價與艙位 / 專利獨佔期 / 鋼價利差 / 去化率 / 電網標案金額),
   不是只說「有正面影響」。
3. **方向＋幅度＋信心**:利多/利空/中性、幅度大小、**資訊強度(A/B)＋信心(高/中/低)**。
**禁止**「對 X 類股有帶動作用」「情緒帶動」「中性偏正」這類沒有機制的空話——那等於沒分析。

每條格式(嚴格遵守,與八段對齊):
【類股｜地區】**(有前情時)半句承接這條線** ＋ [今日事件＋數字＋來源] ＋ [傳導機制] ＋ **[A/B 級・信心:高/中/低]**
(承接語見 R16/R16b:只在上方【進行中的線索】或昨日回顧確有前情時寫,沒有就直接寫今日事件,**不可捏造**。)

範例(A 級,有前情承接＋機制＋分級):
【金融｜台灣】**延續昨日央行理監事會的信用管制討論**——壽險業上半年獲利 3,094 億、淨值衝 4.1 兆新高,受惠台美股雙漲與債券評價回升 [中央社]。壽險投資收益直接墊高國泰金/富邦金的下半年獲利基期,而利率若如市場預期 9 月再升,債券評價利益將部分回吐。**[A 級・信心:中]**
...
4. **影響說明必須具體**：要寫「利多/利空了誰、透過什麼機制、幅度多大」。**禁止**「對 X 類股有帶動作用」「情緒帶動」「中性偏正」這類無機制空話——沒講出機制就等於沒分析。
   ...
   - **金融(壽險/金控)**請扣連這條傳導鏈:美股/美債走勢→壽險投資收益、央行利率→銀行淨息差;能寫出這條鏈才算合格。
     國泰金(2882)/中信金(2891) 的消息**優先入選**,但**輸出中不得說明它們被
     優先處理**——不得出現任何指涉讀者、暗示讀者持股、或暗示本報有一份特定
     關注清單的措辭(見 R15、R15b):【重點公司最新新聞】與
   ...
   - **生技/醫療(寫得具體)**:事件優先序 FDA/EMA 核准或里程碑 > 臨床試驗解盲/進度 > 健保給付 > 併購/授權;機制要明確——新藥上市→專利獨佔期營收、解盲成敗→股價常 ±15–30%、納入健保→營收確定性提升。**禁止**「生技基金看好」「長線可期」這類無事件、無機制的空話。
...
聚焦昨日對台灣資本市場有影響、**而且八段與九段都沒有寫過**的事:
- 台灣總經數據（出口、外銷訂單、CPI）
- 政府政策（產創條例、科專、台美 21 世紀貿易倡議等)、金管會行政措施
- 其餘在地事件(交通/能源/災害對產業的實際影響等)

**排除規則(避免前後段重複)**:
本段與八/九段的範圍原本就會撞 —— 台積電供應鏈屬八段、央行與金融政策屬九段
的金融條目。**同一件事只在一個地方寫完整版**:
- 已寫進八段(科技/半導體供應鏈)或九段(金融/航運/生技…)的事件,
  本段**整條不要出現**;真的必須提到,最多半句並指向該段(例:「詳見九段」)。
- 本段留給「不屬於任何類股條目、但對台灣市場有影響」的在地事件。
- 依上述排除之後若沒有東西可寫,**直接寫「昨日無重大本地新聞」**,
  **不要**把八/九段寫過的搬過來充數 —— 那是重複,不是補充。
...
def _set_analysis_origin(origin: str) -> None:
    """記下這封信的分析**走的是哪一條路**(第十四輪 P0-1)。

    後寫的贏:Luna 特化成功後仍可能落回 legacy(輸出不完整、重試),
    那時最後一次標記才是真的。
    """
    _RUN_MANIFEST.setdefault("llm", {})["analysis_origin"] = _ao.normalize(origin)


def _analysis_origin() -> str:
    """目前記著的出處。沒記過就是 `unknown`,**不會是任何一種成功**。"""
    return _ao.normalize((_RUN_MANIFEST.get("llm") or {}).get("analysis_origin"))


def _fallback_analysis_text(news: list[dict], err: Exception) -> str:
    """LLM 完全失敗時的備援文字。仍提供原始新聞清單與錯誤說明。

    第十四輪 P0-1:**出處在這裡標記,不在四個 return 各補一行。**
    `_call_llm_analysis_impl` 有四條路走到備援(prompt 組裝失敗、Gemini 截斷、
    Gemini 也失敗、全部失敗),逐個補等於留四個會被漏掉的地方 ——
    而漏掉的症狀是帳本把一封沒有模型判斷的信記成某條路徑的成功。
    """
    _set_analysis_origin(_ao.EMERGENCY_FALLBACK)
    top_news = "\n".join(
        f"- [{n['source']}] {n['title']}"
        for n in news[:20]
    )
...
def _call_llm_analysis_impl(quotes: dict, fair: dict, predictions: dict,
    ...
    if _profile == "luna56_xhigh_v1" and DEEPSEEK_API_KEY:
        try:
            ...
            if _text:
                ...
                _set_analysis_origin(_ao.LUNA_SPECIALIZED)
                return _text
    ...
    try:
        ...
        return _fallback_analysis_text(news, e)   # 出處在該函式內標記
    try:
...
def render_html(quotes: dict, fair: dict, predictions: dict, analysis: str,
    ...
    # 批#21(2026-07-18):信件不得出現「使用者…」表述(prompt R15 已禁;
    # 此為 render 防線)。
    # r1(Codex,P1)**確認**:原本是把「使用者」整詞替換成「本報」,於是
    # 「使用者核心觀察」變成「**本報核心觀察**」——**防線自己製造出 R15b
    # 禁止的揭露**(等於公開一份關注清單)。而既有測試還斷言了那個結果,
    # 把缺陷釘成規格。
    # 正解:**整句移除**而不是換個說法。這類句子的功能就是交代入選緣由,
    # 不含任何事實,拿掉不會損失資訊。
    analysis_for_render = _strip_selection_rationale(analysis_for_render)
    ...
    portfolio_risk_html = ""
    # 線索追蹤卡:使用者要求刪除(2026-07-31)。理由是敘事連貫已由「科技板塊
    # 脈動」那種「前情 → 今日進展」的寫法達成,不需要再獨立列一張軌跡卡。
    # **帳本與 prompt 注入保留** —— `_format_story_block` 產生的
    # 【進行中的線索(跨日追蹤)】正是那種寫法的素材來源(前情與軌跡數字),
    # 砍掉它會把使用者真正想要的東西一起砍掉。
...
def _phase_llm_analysis(ctx) -> None:
    ...
    _snap, _cfg = _lc.config_snapshot(
        ...
        shadow_provider="",   # 影子已拆除(2026-08-08 單一模型)
