...
def classify_news_importance(news: list[dict]) -> list[dict]:
    ...
    for n in news:
        ...
        geo_hit = _matches_any(text, GEOPOLITICAL)
        tw_hit = _matches_any(text, TW_POLICY)

        # 評分邏輯：兩個或以上關鍵字命中 → critical；單一命中 → high
...
def render_html(quotes: dict, fair: dict, predictions: dict, analysis: str,
    ...
    if taifex.get("foreign_oi_net") is not None:
        ...
        taifex_html = f"""
        <h2 style="color:#0f172a;font-size:20px;margin:32px 0 12px;padding:8px 14px;background:#e0f2fe;border-left:5px solid #0284c7;border-radius:4px;">五、外資台指期未平倉（領先指標）</h2>
    ...
    if sec_filings:
        ...
        sec_html = f"""
        <h2 style="color:#0f172a;font-size:20px;margin:32px 0 12px;padding:8px 14px;background:#e0f2fe;border-left:5px solid #0284c7;border-radius:4px;">六、主要科技股 SEC 8-K 公告（近 48 小時）</h2>
...
def main() -> int:
    ...
    try:
        news = fetch_news_fulltext(news, max_critical=8)
    except Exception as e:
