...
- **`top_news_analysis` 以六到十則為目標**。當日素材充足而只寫四五則,
  漏掉的不是版面是內容。
  科技之外(金融/航運/傳產/生技)當日有重大新聞時**至少一到兩則** ——
  信裡的「其他類股」段落靠它們才存在。真的不足六則時,在 `data_gaps`
  說明為什麼(例如當日新聞面貧乏),不要硬湊重複的事件。
- **`taiwan_policy` 是台灣政策/主管機關動態的分析**:金管會、央行、
  證交所、稅制、勞動基金、普發現金、產業政策這一類。每一項寫**那件事
  是什麼**(客觀)與**對台股/產業的影響**(判斷),並附 `source_item_id`。
  當日沒有這類新聞就給空陣列 —— 不要拿總經數據或公司新聞充數。
