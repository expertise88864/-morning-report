...
DEEPSEEK_LEGACY_VERSION = 17  # CR-02 2026-09-05:Python 計分缺席時立場未知,禁止 LLM 補算。v16 世界大事 5 條;v15 圍欄規則外移;歷次內容保留於 Git。
...
LUNA_XHIGH_VERSION = 51  # CR-02:Python 計分缺席時明確採未知/null,不准由結構化 LLM 補算。
...
- `EVIDENCE.market.STANCE_PY` 是 Python 已完成的權威計分，stance.score 與
  stance.label 必須抄錄其 total 與 label，禁止自行計算、改判或以新聞分數替代。
- STANCE_PY 缺席、為空或 total 不可得時，權威立場未知：stance.score 必須為
  null，stance.label 必須為「資料不足」，rationale 說明「系統計分缺席」。
  executive_summary 與 portfolio_implications 不得補造本報多空立場或據此加減碼；
  其餘新聞、機制、風險及有條件的觀察照常分析，不把未知寫成中性或零分。
