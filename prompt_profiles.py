...
#: 改七之四鐵則、C 級補上書單廣告類。
#: v5(2026-08-04):使用者再提「還是在堆疊數據、沒有分析影響」。加 R17
#: (Python 排好的表要在立場理由裡被合起來解讀)、七之二 60→90 字且要求
#: 寫得出傳導路徑而不是四個字的抽象標籤。
#: v6(2026-08-04 二次):使用者澄清是**LLM 自己寫的段落**在堆數據。實測八段
#: 10 條有 10 條以方向形容詞作結、0 條說得出量級、0 條跨條關聯。根因是
#: prompt 的**格式模板與兩個範例自己在示範那個毛病**(模板甚至沒有「幅度」
#: 這一格)。改:量級/時間取代方向詞、判斷不出量級要明講、至少兩條要跨條
#: 連結、句式不得雷同,兩個範例整個重寫。
...
#: v4(2026-08-03 晚):同一批可讀性修正的 Luna 版——全中文轉述、
#: 術語白話化、數字要有下文。與 legacy 同批改,兩邊比較才仍然成立。
