...
DEEPSEEK_LEGACY_VERSION = 16  # v16 2026-09-04 使用者:世界大事速覽改 5 條(原 3-5 條;過長重試也改 5 條);  # v15 2026-09-03 全案審查 LM-7:圍欄內的取材指令搬到圍欄外(【圍欄內各段的取材對應與規則】),段標改中性;  # v14 2026-09-03 使用者七項(刪七之五/刪結論卡三行/前言守衛/台股為主/R16c);  # 2026-08-27 使用者七項(0050 行/七之二規則/行事曆解說);  # 2026-08-25 使用者:七之二要「後續可能影響」(長度重試不再壓成一行);  # 2026-08-22 金融龍頭+5; 2026-08-21 r2:選材規則移到圍欄外+無條件(v9 同批修訂)
...
LUNA_XHIGH_VERSION = 50  # v50 2026-09-04 使用者:世界大事速覽改五條(原「約三條」);  # v49 2026-09-03 使用者:整封信只講一個故事(結論卡不得引入正文沒出現的驅動因子)+ 傳導終點改台股;  # v48 2026-09-01:總經發布規則教複數(每一個未駁回×三分支+駁回出口),與判準端對齊
...
- **整封信只講一個故事**:會進結論卡的每一個欄位 —— `stance` 的理由、
  `portfolio_implications.risks` / `.summary` / `.actions_to_consider`、
  `scenario_tree.invalidation_triggers`(它們會被渲染成結論卡的失效條件)、以及
  `executive_summary` —— 只能引用 `key_drivers` / `top_news_analysis` /
  `scenario_tree` 已經寫到的事件;讀者在結論裡第一次看到某個事件,就代表正文
  漏寫或結論編造。`executive_summary` 是 `stance` 的壓縮,立場詞與關鍵事件
  要一致。同一個事件在不同欄位不得有相反方向,除非在後者說明為何不同;
  `narrative_delta` 說的轉變理由必須就是 `stance` 用來解釋轉向的那一條。
  傳導鏈的終點寫台股/加權/2330/相關類股 —— 不要以 00662 為主詞;它的價位在
  表格裡就好。
...
- **`taiwan_policy` 是重大政策深度解析**:行政院公報（EVIDENCE 的
  ...
  **公報的 `source_item_id` 照抄該筆的 `citation_id`**
  ...
  是同一種東西;
  政策新聞則照樣寫那則新聞的 id。
- **`world_events` 是股市之外的世界**（五條）:外交、戰爭、科技治理、
