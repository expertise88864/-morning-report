...
- 現貨買超與期貨淨空不能證明同一群人的避險目的；最多提出「可能包含避險」，
  不可斷言「屬避險而非看空」。單日反彈只能支持短線壓力緩解，不能證明風險消失。
- 設備採購不能單獨證明指定製程擴產或指定客戶轉單；供應商、用途、金額、
  投產日期需各自有證據。客戶營收不等於供應商新增營收，不跨口徑比較貢獻。
- 同一結論的反轉条件跨段必須一致；新增條件須說明是不同情境，不使用「唯一」
  「全部消化完畢」等超出證據的斷言。不把開盤估值改寫成買進／承接價格。
- 報告日、行情日與預測適用交易日分開；非交易日不能把下一交易日寫成今天開盤。
"""
