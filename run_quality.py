...
# 期限原語**與判準本體**都住在 `delivery_sla`(2026-09-05 第二批搬出)。
...
def assess(manifest, *, mode: str = "watchdog",
    ...
    # 而 `news_analyzed` 仍寫 18 —— 指標對得上模型、對不上讀者。渲染端現在把
    # 每一則的結果記進 `llm.news_render`。(9/4 我把「九段整段不見」也歸到這裡,
    # 那是錯的 —— 主因是 9f 的保險絲,9/5 查明:18/18 都渲染了,九段照樣不見。)
    ...
    _cap = _dig(m, "llm", "analysis_cap", default={}) or {}
    if _cap and _safe_int(_cap.get("kept")) < _safe_int(_cap.get("chars")):
        _lost = [str(x) for x in (_cap.get("lost_sections") or [])]
        add("analysis_capped", "defect",
            f"分析文字 {_safe_int(_cap.get('chars'))} 字超過保險絲 "
            f"{_safe_int(_cap.get('limit'))},信裡只剩 {_safe_int(_cap.get('kept'))} 字"
            + (";整段消失:" + "、".join(_lost[:6]) if _lost else ""))

    # ---- 10a-2. evidence packet 的邊界正規化(r18 外審 P2;原本夾在 SLA 那段中間)
    #
    ...
    # ---- 10a. **09:00 SLA** —— 判準本體住在 `delivery_sla.assess_delivery`
    # (2026-09-05 搬出,工單見 tests/test_module_size_freeze.py)。這裡只委派;
    # 兩處都有判定就會分歧,而分歧的症狀是「同一份 state 兩種結論」。
    assess_delivery(m, add, digest=digest)
