...
#: 而且是靜默的**。
_WRAP_EVENT_WORDS = ("財報", "業績", "展望", "法說", "營收", "獲利", "訂單",
                     "併購", "收購", "增資", "配息", "解盲", "核准", "裁罰",
...
def _has_company_event(title: str) -> bool:
    ...
    _season = _re.compile(r"^(?:會|周|週)?(?:[週周季月]|旺季|行情)")
...
def is_market_wrap(title: str) -> bool:
    ...
    (文中提到那檔股票),它的「數字事實」抽出來是道瓊漲點(260 → 500),
    於是兩則不同日的大盤總結被並列成「一條演進中的線索」——毫無意義,
    而且排在「線索追蹤」卡第一條。

    **強/弱兩級判準**(r1 Codex + 自驗):
      - 欄目型標題(〈台股盤後〉…)或強標記(操盤筆記/盤中速報/台股收盤…)
        → 一律視為總結。第一版我用單一清單 + 事件詞豁免,結果
        「本週操盤筆記:Fed決策、AI支出大戶**財報**」因為含「財報」而被放行。
      - 弱標記(盤後/盤前/盤中/收盤/開盤)→ 只有在**沒有任何事件詞**時才算總結;
        「鴻海盤後公告 斥資100億擴廠」「恩智浦盤後下跌…業績超預期」都是真新聞。
      - r1 Codex(P2):移除了泛用的「快報」——「【財報快報】台積電獲利創高」
