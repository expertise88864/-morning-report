...
#: 欄目型標題的前綴(〈台股盤後〉…)
_WRAP_PREFIX = ("〈", "【", "《")
#: 欄目名裡出現這些詞 = 綜覽欄目(不限哪個市場:〈能源盤後〉也是)
_WRAP_COLUMN_WORDS = ("盤後", "盤前", "盤中", "收盤", "開盤", "速報", "快訊")
...
def is_market_wrap(title: str, known_names=()) -> bool:
    ...
    **這是第七版。** 前六版都在加關鍵字與詞性後綴,每一版都被下一個反例打破:
    ...
    if t[:1] in _WRAP_PREFIX and any(
            m in t[:14] for m in _WRAP_MARKET_SUBJECTS + _WRAP_COLUMN_WORDS):
        return True
