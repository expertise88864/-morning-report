...
# 2026-09-03 使用者七項(刻意變更):刪七之五多空交鋒、刪結論卡第 4-6 行與其括號
...
# 2026-09-04 使用者(刻意變更):世界大事速覽「3-5 條」→「5 條」;過長重試「最多 3 條」→
# 「仍要 5 條」。逐行 diff 過,只有這兩處字面動了。
# CR-02 2026-09-05:刻意修正未知模式,禁止 LLM 自算;有 Python 權威時抄錄規則不變。
LEGACY_PROMPT_SHA256 = (
    "fd84a845685fa4ed9659da601b43064eca2015b968a39cad03511345dcb5cd79")  # 09/24: qualify TAIFEX aggregate OI inference.
...
def test_the_legacy_deepseek_prompt_is_byte_frozen():
    ...
    assert '自產報時起精確 48 小時內、有明確台北時間的重要行事曆事件' in prompt
    assert '不得改寫成「無重大事件」' in prompt
