...
MODULE_CEILINGS = {
    ...
    # 重點:誤併比漏併危險。實測 108。
    "cross_lang.py": 125,
    # 分析面縱深:昨日觀點閉環(存 key_drivers → 明天掛在事件群上 →
    # 驗重述)。同日重跑守衛與實體比對佔大半。實測 153。
    "analysis_recap.py": 175,
    ...
    # 分析面縱深:+ 重述檢查(與昨日觀點高度重複的延續事件敘述退回
    # 加深)。overlap 本體在 analysis_recap,這裡只有接線。實測 465。
    "analysis_depth.py": 470,
