...
MAIN_MODULE_LINE_CEILING = 26_485  # 外審尺寸修正:finalize 移入 assembly,移除重複呼叫。 # 2026-09-06:郵件尺寸純函式抽至 email_content_audit;refactor_audit ALL-CLEAR,手機/完整性僅接線。前值 26_494。 # 2026-09-04 資金輪動膠囊渲染抽到 render_utils(表格)—— 抽出**往下調**(棘輪政策);前值 26_498  # 2026-09-03 全案 code review 36 條修正中落在主檔的 21 條(ST-1/2、DL-3/5/6/7/8/9、FR-1/3、LM-1/5/7/8/9/10/11、QT-1/2、TC-2 的字典字面值)—— **全部是 bug fix / 宣稱對齊,沒有新功能**;共用工具(圍欄中和、上一版輸出圍欄、計費判準)都放在葉模組;+170 行,餘裕歸零。  # 前一值 26_308:2026-09-03 Codex r1 兩條 bug fix(+7):一週表按日期對齊(fetch 會跳過壞掉的那天)、舊章節照習慣吐回來時渲染端確定性移除;現況 26,308。  # 2026-09-03 使用者七項:刪七之五/結論卡三行/前言守衛/警特報縣市/一週天氣表 —— 淨減 8 行,**往下調**(棘輪政策)。  # 2026-09-03 r18 Codex deep P1:manifest 在卻沒登錄的既有分區不得當『全新分區』簽名(兩個入口都要擋)—— **bug fix 的 +6 行**;現況 26,305。  # 2026-09-03 r18 外審 P1:model_history manifest 壞掉不得被當成「沒有 manifest」(讀不動就不寫、留痕、本班跳過)—— **bug fix 的 +9 行,不是新功能**;現況 26,298。  # **凍結在現況**(2026-09-03 r17 架構外審 P1)。
...
MODULE_CEILINGS = {
    ...
    # 實測 1011。
    "event_identity.py": 1275,  # 2026-08-26 r2:組織身分+縮寫大小寫規則;現況 1191  # 2026-08-26 r2:被動由「後面有沒有受詞」判;現況 1144  # 2026-08-26 外審:制裁受詞位置剖析(sanction:Oil);現況 1120  # 2026-08-22 r1 外審 P2:_subjects_meet 套 usable_alias(裸數字別名);現況 1041  # r2:主體正規化走單一權威+別名納入標題比對;現況 1025
    # 動作表(宣告式資料 + 純字串函式)。實測 100。
    # Commit 2:ASCII 關鍵詞詞界 + 放行複數(硬化,非已確認缺陷)。實測 151。
    # 第二十七輪外審 P1-5:法域的宣告表(法域永遠不是可交易標的)
    # **實測 191 行**。
    # 2026-08-18 外審 P2-1:`cyberattack` 的詞彙表補上生產實際的寫法
    #(間諜軟體 / 駭客攻擊 / spyware …)—— 型別層也讀這一份,
    # 兩層曾經自己互相矛盾(一層 cyberattack、一層 geopolitical)。
    # 實測 249 行。
    "event_actions.py": 300,  # 2026-08-22 外審 P2-3:ACTION_EVENT_TYPE 宣告表;現況 270
    ...
    "run_manifest.py": 450,  # 2026-08-22 r1 外審 P2:corrupt 明細由 recorder 擁有(整段覆寫會吃掉留痕);現況 419   # 2026-08-07:luna_path_failure 加 traceback 記錄(+13 行)
    ...
    # Batch A:lineage 端到端(match_lineage/簽章候選集合/世系接線)
    "evidence_packet.py": 850,  # 2026-08-24 外審 P1:gazette 專用 resolver(標題撐不住政策解讀);現況 815  # 2026-08-24 外審 P1:切片要帶語意(tension 左右/容器攤平);現況 730  # 2026-08-22 外審 P1-1:修補輪的證據切片(ID→內容的對照歸 packet 模組);現況 635
    ...
    # 2026-08-09 外審:跨語言前置條件要認法域的中英對應
    # (`entity_alias` 刻意不收國家)。**實測 225 行**。
    "news_clusters.py": 235,
    # Commit B 新增:來源註冊表(誰跟誰是同一個編輯台)與
    # 兩階段抓取計畫(全文預算逐事件群分配)。兩個都是純規則層。
    # 第二十三輪 P1-4:ASCII 別名 token 邊界 + 未知來源去重。實測 251。
    # 深度優化(橫向):+ 聚合器標題尾綴的發布者解析(`title_publisher` /
    # `bare_title`)。欄位空時發布者藏在「標題 - 發布者」裡 —— 先前整則
    # 記成 aggregator_only,實際解得出來。實測 288。
    # 2026-08-09 P2:未知發布者以註冊網域去重(同一站三種寫法算三個來源)。**實測 325 行**。
    "source_registry.py": 370,
    # 深度優化(縱向):延燒事件優先(`_continuing`/`timeline_map`)——
    # 延續事件要寫增量,增量需要全文細節。實測 187。
    # 2026-08-09 P2:可抓候選數,且「已經有全文」與「沒有連結」分開記
    # (兩種零的後果相反)。**實測 207 行**。
    # 2026-08-17 外審:規劃器跑在正規化之前,實體要在分群前補上(否則生產 402 則新聞 402 群,全文額度被同一事件的轉載吃掉)。實測 219。
    "fetch_plan.py": 230,
    ...
    # 就不存在)。實測 753。
    # 2026-08-17 外審 P2-1 r2:`ledger_triggers()` —— 存檔失敗時要問
    # 「磁碟上實際還追著誰」(舊帳本通常完整保留;傳空集合會少報)。
    # 2026-08-17 外審 r2 P2-1:`trigger_key()` —— 觀察點的**身分**是完整
    # 文字的雜湊,顯示才是截到 120 字。兩者共用一個函式時,前 120 字相同
    # 但結論相反的兩條(跌破 1100 / 突破 1200)會被壓成一條。
    # 2026-08-17 外審 r3:帳本要存**完整**文字(`trigger_full`)—— 身分拆開
    ...
    # 2026-08-14:總經發布每驅動至多一筆、標題級發布優先(PPI 日全場
    # 被歸成 us_inflation → 47 個「發布」結構上不可能滿足);外審兩輪
    # 補發布形狀層+純發布層(反應語句降級、數據類要求數值、決議豁免)。
    # 實測 308。
    "event_graph.py": 315,
    ...
    "entity_alias.py": 175,  # 2026-08-22 外審 P1:組代表成為持久鍵的一部分(重複組清理+契約說明);現況 101
    ...
    # 2026-09-03 r19:JSON 的數字語意集中到 `is_json_number()`(NaN/inf
    # 不是合法 JSON,而且比較運算全 False 會繞過 bounds)。150→175。
    "json_contract.py": 175,
    ...
    # 2026-08-17 外審 P2-1 r1:trigger 比對要用帳本的正規形式
    #(截到 WATCH_CHARS)—— 拿全文比會把超長但已收下的標成一次性。實測 399。
    # 2026-08-18 使用者定案:第八段依產業拆「科技類股 / 其他類股」,
    # 保留事項獨立成子段(掛在其他類股底下會讓讀者以為只有那一類有缺)。
    # 加的是分組、三個子標題與 `_blocks`(條目不再整塊掛在一個 bullet
    # 底下)。實測 453 行。
    # 2026-08-18 第二批:五段併成「九、今日市場關注與預測」(逐段的
    # 去重、子段區塊化),保留事項的識別碼換標題。實測 540 行。
    "analysis_render.py": 640,  # v22 macro 讀 analysis 子欄位 + r2 敘事原文依 ID 取回;現況 579
    # 第二十四輪 P1-12:標的的**型別身分**(canonical id + 範疇)。
    # 先前沒有「標的」這個型別 —— 判斷散在四處各憑字串形狀猜,而白名單
    # 被當成「與這件事有關」的免死金牌。實測 71 行。
    # 第二十八輪外審 P1-2:宣告清單(未知大寫字串不再算標的)
    # **實測 141 行**。
    "instrument_registry.py": 230,
    # 2026-08-18:「什麼算科技類股」的**唯一一份判準**。第八段改成依
    # 產業拆兩段之後,渲染層與「補非科技類股新聞」問的是同一個問題 ——
    # 兩份集合分歧會讓同一檔股票在兩處被分到不同類股。實測 57 行。
    "industry_class.py": 110,
    # 2026-08-18:外國個股的一句側寫。台股那半有 TW0050_CONSTITUENTS,
    # 外國沒有資料源 —— 這是宣告,與 sector_map.EDGES 同一種東西。
    # 實測 88 行;它會隨涵蓋面成長,上限給到 200。
    "company_profiles.py": 200,
    # 2026-08-18 P1-1:一次性的 state 清理(帳本/時間軸的錯歸因)。
    ...
    "subject_identity.py": 230,  # 主體身分單一權威(2026-08-20 P1-2);現況 114 —— 加主體改表,不該長邏輯  # 第三輪 P1-2:legacy unverified 重驗+信任層級;現況 306
    # 2026-08-18 P1-2:三家 provider 共用的 completion contract。
    # adapter 只做對照,不自己發明政策 —— 三套政策一定會再漂。
    # 實測 125 行。
    "completion_contract.py": 200,
    ...
    # 2026-08-17 使用者定案(敘事為主):鏈壓成一行要處理「鏈斷掉不假裝
    # 連續」、句子接合(先前接出「。、」)—— 收起來的欄位減不了行數,
    # 判斷邏輯反而變多。實測 229。
    # 2026-08-18 使用者定案:小標題回到「公司(代號,簡介):昨天發生
    # 什麼事」—— 加的是主體判定(編輯標註實體 → universe 查名稱/產業/
    # 簡介 → registry 判個股)、退化簡介不重複公司名、標題開頭去重。
    # 逐標的的方向/幅度標籤同批拿掉。實測 341 行。
    # 2026-08-18 第三次校正:公司側寫、發布者、佐證等級與信心的渲染。
    # 實測 412 行。
    "analysis_render_depth.py": 490,
    ...
    "analysis_validate.py": 1_230,  # 2026-09-04 Codex P2:world_events 空 what/why_it_matters 要擋(+7,bug fix);前值 1_220,  # 2026-09-04 實信:空 why_it_matters 的卡片靜默消失 —— 驗證器擋(+9,bug fix);前值 1_210,  # 2026-09-03 全案審查 LM-2/3/4:空欄位檢查、_q 截斷、weakly_corroborated 共用述詞(+59,全是 bug fix);前值 1_150,  # v22 敘事/總經證據檢查;現況 1089,    # 2026-08-08 外審 P1-8:key-driver 反證也要驗
    ...
    # 實測 464 行。
    # 2026-08-19 第四批:加深身分保 legacy 骨架各段。實測 483 行。
    "analysis_depth.py": 850,  # 2026-09-04 使用者:世界大事五條的守衛(WORLD_EVENTS_TARGET + advisory,+21);前值 830,  # 2026-08-25 外審 P2:逐段可行性(拆掉全域門檻)+素材面只算認得出主體的;現況 793  # 2026-08-24 外審 r3:涵蓋率缺口的雙向規則(該在不在/不該在還在);現況 763
    ...
    # CI #506:再加自發現缺口的 `canonicalize_gap_ids`(與張力前綴
    # 同形狀的第二個正規化;判準集中在本模組)。
    "tension_refs.py": 210,
}
